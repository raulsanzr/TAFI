...
@njit
def calculate_distance(vaf2,nr_of_repeats,purity,S,C,cov,min_reads,xdata,y_prob):
    '''
    Calculates the wasserstein distance (averaged across nr_of_repeats times) between the observed and simulated VAFs.
    '''
    ...
    for repeat in range(nr_of_repeats):
        ...
        if len(vaf1) > 10: # if there are at leat 10 mutations simulated
            r_nd5[repeat] = wasserstein_distance(vaf1, vaf2)
...
def run_fit(test_model,cov,min_reads,max_steps,observed_vaf,collected_data_size,xdata,y_prob,pred_purity=None,pred_C=None):
    ...
    for i in range(0,max_steps):
        for counter,current_purity,current_S,current_C in zip(range(0,collected_data_size),pur_init,S_init,C_init):
            ...
            proposed_dist=calculate_distance(observed_vaf,nr_of_repeats,new_purity,new_S,new_C,cov,min_reads,xdata,y_prob)
